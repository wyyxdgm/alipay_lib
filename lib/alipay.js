...
Alipay.prototype.create_direct_pay_by_user_return = function(req, res) {
    ...
    alipayNotify.verifyReturn(_GET, function(verify_result) {
        if (verify_result) { //验证成功
            ...
            if (alipayNotify.alipay_config.create_direct_pay_by_user_return_url_site) {
                res.redirect(alipayNotify.alipay_config.create_direct_pay_by_user_return_url_site);
            } else {
                res.send('success');
            }
        } else {
